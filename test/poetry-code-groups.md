<!--«tabNameBrackets»-->
<!--\*\*poetryBold\*\*-->
<ru>Как видно, в целом код действительно получился настолько читабельным, что наши текущие комментарии...</ru>
<uk>Як бачимо, в цілому код дійсно став настільки читабельним, що наші поточні коментарі...</uk>
<en>As you can see, in general, the code really becames so readable that our current comments...</en>
...
<ru>совсем не нужны:</ru>
<uk>зовсім не потрібні:</uk>
<en>are no longer needed:</en>
...
<ru>Этих основных команд, поддержки функций «Autocomplete» и «Quick Fix» от IDE, мужества покопаться в коде, и, в конце концов, официальной документации – должно быть вполне достаточно, чтобы начать писать первые автотесты вместе с библиотеками типа Selenide.</ru>
<uk>Цих основних команд, підтримки функцій «Autocomplete» та «Quick Fix» від IDE, мужності покопатись в коді, і, зрештою, офіційної документації – має бути цілком достатньо, щоб почати писати перші автотести разом з бібліотеками типу Selenide.</uk>
<en>These basic commands, support for functions “Autocomplete” and “Quick Fix” from IDE, the courage to dig into the code, and finally, the official documentation - should be quite enough to start writing the first tests with the library of “Selenide” type.</en>
